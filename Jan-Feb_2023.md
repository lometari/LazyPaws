...
[Frogger Kottans](https://github.com/kottans/frontend/blob/2022_UA/tasks/js-oop-frogger.md)
сломал мне мозг. В такие моменты невольно задумываешься, а стоило ли такому тупичке браться за программирование?..

Но, отступать, как говориться нельзя, поэтому все свое недоумение оставлю здесь.
...
```
![Вывод в консоль без имен переменных](./img/01.2023/frogConsole1.jpg)

Выводит в консоль и переменную название класса. Не поняла пока почему меня это беспокоит. Наверное, просто тупнячок нашел.

**2. Создание player.**
Для того, чтобы описать методы player его обязательно нужно создавать конструктором? Даже если он у нас будет только один?

**3. Совсем туго с пониманием как отрабатывает движок по отрисовке...**
Почему пустой метод Player "запускает" отрисовку поля? (Без видео вообще бы не шмогла бы...)
```js
...
```
<details>
    <summary>Внимание, ответ:</summary>
    Видимо отрисовка начинает работать, как только описаны все методы из этого места в  engine.js

![hhh](./img/01.2023/frogEngineRender.jpg)
</details>


**4. Как высчитать значение высоты и ширины спрайтов**, чтобы, в свою очередь, высчитать стартовое размещение и пределы перемещения их на канве?

**5. Что нужно сделать:**

    - ограничить игрока размерами поля
    - сбрасывать на стартовую позицию, если дошел до воды
    - заставить зарендериться врагов
    - понять как реализовать столкновение игрока и врагов
    - сделать врагам разную скорость

****
### **## Ссылки:**
Видео, которое помогло сдвинуться с места  
